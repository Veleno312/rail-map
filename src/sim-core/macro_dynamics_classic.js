 ...
 */
function calculateAccessibility(region, networkState, travelTimes) {
 ...
 */
function macroDerivatives(macroState, transportState, eventEffects) {
  const derivatives = {};
 ...
 */
function updateDemandMultipliers(macroState) {
  for (const regionId in macroState) {
    const region = macroState[regionId];
 ...
 */
function applyStabilityConstraints(macroState) {
  for (const regionId in macroState) {
    const region = macroState[regionId];
 ...
 */
function generateMacroReport(macroState, tick) {
  const report = {
    tick,
    timestamp: Date.now(),
    regions: {},
 ...
 */
function validateMacroState(macroState) {
  const errors = [];
  const warnings = [];
...
window.initializeMacroVariables = initializeMacroVariables;
window.calculateAccessibility = calculateAccessibility;
window.macroDerivatives = macroDerivatives;
window.updateDemandMultipliers = updateDemandMultipliers;
window.applyStabilityConstraints = applyStabilityConstraints;
window.generateMacroReport = generateMacroReport;
window.validateMacroState = validateMacroState;
