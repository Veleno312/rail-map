...
if (line.nightService) return cost * 1.10;
...
// Service window
const serviceStartMin = Number(line.serviceStartMin ?? 0);
const serviceEndMin   = Number(line.serviceEndMin ?? 24*60);
let serviceWindowMin = serviceEndMin - serviceStartMin;
if (serviceWindowMin < 0) serviceWindowMin += 24*60;
...
const endMin   = Number(line.serviceEndMin ?? 1410);
...
if (!String(s.schemaVersion || "").startsWith("rail-luti-scenario@"))
...
if (s.networks?.rail?.lines && Array.isArray(s.networks.rail.lines)){
  ...
  for (let i=0;i<rawLines.length;i++){
    ...
    const ln = {
      ...
      serviceEndMin: clamp(Math.round(Number(rl.serviceHours?.endMin ?? 1410)), 1, 1440),
...
const line = state.lines.get(state.activeLine);
if (!line) return;
line.serviceEndMin = hhmmToMin(v);
ui_lineRecalcTrainsFromDepartures(line);
updateUI();
}
...
const start = Number(line.serviceStartMin || 0);
const end = Number(line.serviceEndMin || 1440);
...
updateUI();
showToast(state.lineBuildMode ? "Add-stops ON" : "Add-stops OFF", "info");
}
