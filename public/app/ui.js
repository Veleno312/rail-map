  ...
  if (state.lines instanceof Map) {
    ...
    for (const line of state.lines.values()) {
      ...
      if (!Array.isArray(line.stops) || line.stops.length < 2) {
        ...
        warnings.push(`Line "${line.name || line.id || "unnamed"}" needs ≥2 stops.`);
  ...
  return `
    <div class="section">
      ...
      <div class="hint" style="font-size:11px;line-height:1.4;">
        These controls edit the same comarcas/demand heat/underserved/catchment overlays that back the station-first assignment in Replace.txt, and the recompute action reruns the cells→stations demand map.
      </div>
...
function renderInvestmentPanel(opts = {}){
  ...
  const companyOptions = companies.map(c => `
    ...
    <option value="${c.id}">${escapeHtml(c.name)} — ${formatCurrency(Number(c.sharePrice || 0))}/share</option>
    ...
    ${activeNode ? (() => {
      ...
      return `
        ...
        <div style="font-weight:900;color:#334155;font-size:12px;margin-bottom:6px;">
          ...
          ${escapeHtml(activeNode?.nodeId || activeNodeId)} • ${escapeHtml(state.nodes?.get(activeNodeId)?.name || "")}
...
function updateUI(){
  ...
  <div style="font-weight:1000;color:#0f172a">${ui_lineDisplayName(l)}${l.circular ? " ⟳" : ""}</div>
...
...editorLines.map(l => `<option value="${l.id}" ${l.id===state.activeLine ? "selected" : ""}>${ui_lineDisplayName(l)} • ${l.type}</option>`)
...
const dynamicsTabHtml = (showPrimaryPanel && state.activeTab==="dynamics" && state.primaryTab !== "stations")
    ...
    return `
      ...
      <div class="item" style="cursor:default;">
        ...
        <div style="color:#64748b;font-weight:850;font-size:12px;line-height:1.35;">
          ...
          ${daysLeft}d left • ${jobProgressPct(j)}% • ${formatCurrency(jobCostLeft(j))} remaining
  ...
  const reportRunIdShort = escapeHtml(
    ...
    reportRawRunId.length > 32
      ...
      ? `${reportRawRunId.slice(0, 16)}…${reportRawRunId.slice(-12)}`
...
const topCellsDetailHtml = underservedEntries.slice(0,3).map(([cellId, value]) => {
  ...
  return `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-top:4px;font-size:12px;">
      <span>${escapeHtml(label)} — ${amount} underserved</span>
...
const unusedStationsListHtml = unusedStations.slice(0, 20).map(station => `
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
          <div style="flex:1;">
            ...
            <div class="hint" style="font-size:11px;margin-top:2px;">Serves ${fmtNum(Math.round(station?.coveredPopulation || 0))} people · ${fmtNum(lineCounts.get(station.id) || 0)} lines</div>
          </div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
          <div style="min-width:0;">
            ...
            <div style="font-size:12px;color:#64748b;">
              ${escapeHtml(String(station.source || 'auto').toUpperCase())} · ${linesCount} lines · ${station.active ? 'Active' : 'Disabled'}
            </div>
          </div>
  ...
  return `
    <div class="section">
      ...
      <div style="line-height:1.6;">
        ...
        <br>${kv('Active', fmtNum(active))} · ${kv('Disabled', fmtNum(disabled))}
...
const overlaySectionHtml = `
      <div class="section">
        ...
        <div class="hint" style="margin-bottom:8px;font-size:11px;">${infraLabel} · ${infraHint}</div>
...
const selectionInspectorHtml = (() => {
  if (selectedCell) {
    ...
    const topAllocationsHtml = allocations.length
      ? allocations.slice(0,3).map(entry => {
        ...
        return `<div style="font-size:12px;line-height:1.4;">${escapeHtml(label)} · ${sharePct}% · ${fmtNum(Math.round(entry?.pax || 0))} pax/day</div>`;
      }).join("")
  ...
  if (!selectedStationId) {
    ...
    const cityAlloc = cityKey ? (state.cityStationAllocation?.get(cityKey) || []) : [];
      ...
      ? cityAlloc.slice(0, 3).map(entry => {
        ...
        return `
          <div style="font-size:12px;line-height:1.4;display:flex;align-items:center;gap:6px;">
            <span>${escapeHtml(name)} — ${fmtNum(Math.round(entry.share || 0))} ppl (${sharePct}%)</span>
...
const pendingInfraStatus = hasPendingInfraChanges
  ? `<span style="color:#b45309;font-weight:900;">Pending updates ready – commit to apply</span>`
...
` : `
  ...
  const windowControlsHtml = `
    ...
    <div class="window-controls">
      ...
      <button class="window-control" title="${windowControlLabel}" onclick="${windowControlAction}">▁</button>
  ...
  const panelMinimizedContent = `
    ...
    <div class="panel-minimized">
      ...
      <div>
        ...
        <div style="font-weight:1000;color:#0f172a;">${activePrimaryLabel} • ${activeSubLabel} minimized</div>
  ...
  if (isProductionTab) {
    ...
    html += `
      ...
      <div style="font-weight:900;color:#334155;font-size:12px;line-height:1.4;">
        ...
        Serves: ${fmtNum(servedPop)} people${coveredCities ? ` across ${fmtNum(coveredCities)} cities` : ""} · ${coverageKm.toFixed(1)} km radius
