...
function uiMode_compute(){
  ...
  if (state.primaryTab === "scenario") {
    ...
    return {
      ...
      detail: "Scenario and cache controls",
...
const primaryTabs = [
  ...
  { id: "production", label: "Industry", icon: "??" },

  { id: "population", label: "Population", icon: "??" },

  { id: "stations", label: "Stations", icon: "ðŸš‰" },
...
const OFFLINE_MANIFEST_URL = "/data/offline/manifest.json";
const OFFLINE_PACK_URL = "/data/offline/packs/scenario-pack.json";
function getPrimaryMode(){

  if (!state) return "production";

  if (state.primaryTab === "production" || state.primaryTab === "population"){
    state.primaryMode = state.primaryTab;
  }

  if (!state.primaryMode){
    state.primaryMode = state.primaryTab || "production";
  }

  return state.primaryMode;
}
function primaryModeIsProduction(){

  return getPrimaryMode() === "production";
}
function isScenarioPrimaryTab(){

  return state?.primaryTab === "scenario";
}
...
const rawStopIds = Array.isArray(line.stops) ? line.stops.map(id => String(id)) : [];
const stopSet = new Set(rawStopIds);
const orderedStopIds = [];
const orderedSeen = new Set();
if (Array.isArray(line.pathNodes)) {
...
const allowedConnTypes = primaryModeIsProduction()
...
const stopEls = stops.map((s) => {
  ...
  const connRow = connIcons

    ? `

      <div style="position:absolute;left:calc(100% + 8px);top:50%;transform:translateY(-50%);display:flex;gap:4px;align-items:center;white-space:nowrap;">
  ...
  return `
    ...
    <div style="position:absolute;left:${x}%;top:${y}px;transform:translate(-50%,-50%);z-index:2;cursor:pointer;" onclick="ui_lineDiagram_stopClick('${s.id}')">
      ...
      <div style="position:absolute;right:calc(100% + 10px);top:50%;transform:translateY(-50%);text-align:right;max-width:120px;font-size:9px;font-weight:900;color:#334155;line-height:1.1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
  ...
  return `
    ...
    <details class="debug-drawer" id="debugDrawer">
      ...
      <div class="debug-block">
        ...
        <label class="debug-meta" style="display:flex;gap:6px;align-items:center;">

          <input type="checkbox" ${state.debug?.perf ? "checked" : ""} onchange="ui_setPerfLogging(this.checked)">

          Perf logging (timed demand + overlay updates)
...
/* exported ui_setPerfLogging, ui_commitPendingChanges */
/* eslint-disable-next-line no-unused-vars */
function ui_setPerfLogging(enabled){
  if (!state.debug) state.debug = {};
  state.debug.perf = !!enabled;
  if (typeof updateUI === "function") updateUI();
}

/* eslint-disable-next-line no-unused-vars */
function ui_commitPendingChanges(){
  if (typeof applyPendingChanges === "function") {
    applyPendingChanges({ showToastMsg: true });
  } else if (typeof showToast === "function") {
    showToast("No pending change handler available", "warning");
  }
  if (typeof render_overlay === "function") {
    try {
      render_overlay();
    } catch (err) {
      console.warn("render_overlay failed", err);
    }
  }
}
...
function ui_scenarioPanelHtml(){
  ...
  if (state.offlineManifest?.nodes) {
    detailRows.push(`Nodes: ${state.offlineManifest.nodes}`);
  }
  if (state.offlineManifest?.tracks) {
    detailRows.push(`Tracks: ${state.offlineManifest.tracks}`);
  }
  const offlineGeneratedAt = state.offlineManifest?.generatedAt
    ? new Date(state.offlineManifest.generatedAt).toLocaleString()
    : "";
  const extraTags = [
    ...
    offlineGeneratedAt ? `Generated ${escapeHtml(offlineGeneratedAt)}` : ""
  ].filter(Boolean);

  const extraInfo = extraTags.length

    ? `<div class="hint" style="margin-top:4px;font-size:11px;color:#64748b;">${extraTags.join(" Â· ")}</div>`
  ...
  return `
    ...
    ${overlayQuickControlsHtml()}

  `;

}

function overlayQuickControlsHtml(){
  const mapLayers = state.mapLayers || {};
  const toggles = [
    { key: "showComarcaBorders", label: "comarca borders" },
    { key: "showDemandHeat", label: "demand heat" },
    { key: "showUnderserved", label: "underserved heat" },
    { key: "showCatchments", label: "catchment lines" }
  ];
  const toggleButtons = toggles.map(t => {
    const isOn = !!mapLayers[t.key];
    const label = isOn ? `Hide ${t.label}` : `Show ${t.label}`;
    return `<button class="btn secondary" style="padding:4px 10px;margin-top:0;" onclick="setMapLayerOption('${t.key}', ${isOn ? "false" : "true"})">${label}</button>`;  
  }).join("");
  return `
    <div class="section">
      <div style="font-weight:1000;color:#0f172a;margin-bottom:6px;">Station-first demand overlays (Replace.txt)</div>
      <div class="hint" style="font-size:11px;line-height:1.4;">
        These controls edit the same comarcas/demand heat/underserved/catchment overlays that back the station-first assignment in Replace.txt, and the recompute action reruns the cellsâ†’stations demand map.
      </div>
      <div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;">
        ${toggleButtons}
      </div>
      <div class="row" style="gap:6px;margin-top:6px;flex-wrap:wrap;">
        <button class="btn secondary" style="padding:6px 12px;margin-top:0;" onclick="ui_openNetworkOverlays()">Open network overlays</button>
        <button class="btn secondary" style="padding:6px 12px;margin-top:0;" onclick="recomputeDemandModel()">Recompute demand</button>
      </div>
      <div class="hint" style="font-size:10px;margin-top:4px;line-height:1.4;">
        Scenario and Economy views share this overlay state, so editing stations/tracks/lines anywhere and recomputing keeps the map consistent with the station-first core.
      </div>
    </div>
  `;
}
...
async function ui_fetchOfflineManifest(){
  return loadJSON(OFFLINE_MANIFEST_URL);
}

async function ui_fetchOfflinePack(){
  return loadJSON(OFFLINE_PACK_URL);
}

async function ui_loadOfflineScenario(){
  const manifest = await ui_fetchOfflineManifest();
  if (!manifest || !manifest.datasetVersion) {
    throw new Error("Offline manifest missing metadata");
  }
  const pack = await ui_fetchOfflinePack();
  if (!pack) {
    throw new Error("Offline scenario pack not found");
  }
  const payload = ui_cloneScenarioPayload(pack);
  if (!payload) {
    throw new Error("Offline scenario payload empty");
  }
...
function ui_updateScenarioCacheStatus(){
  ...
  if (state.offlineManifest) {
    const manifest = state.offlineManifest;
    const stats = [];
    if (Number.isFinite(manifest.nodes)) stats.push(`${manifest.nodes} nodes`);
    if (Number.isFinite(manifest.tracks)) stats.push(`${manifest.tracks} tracks`);
    if (Array.isArray(manifest.tiles)) stats.push(`${manifest.tiles.length} tiles`);
    const generatedAt = manifest.generatedAt ? new Date(manifest.generatedAt).toLocaleString() : null;
    el.textContent = `${modeLabel} - Offline manifest ${manifest.datasetVersion || "n/a"}${stats.length ? ` (${stats.join(", ")})` : ""}${generatedAt ? ` (generated ${generatedAt})` : ""}`;
    return;
  }
  if (typeof offlineCache_getLatestManifest !== "function") {
...
async function ui_toggleOnlineMode(){
  const preferOnline = !state.onlineMode;
  state.onlineMode = preferOnline;
  if (!state.onlineMode) {
    try {
      const manifest = await ui_loadOfflineScenario();
      showToast(`Offline mode enabled (${manifest.datasetVersion || "offline"})`, "success");
    } catch (err) {
      state.onlineMode = true;
      showToast(`Offline mode unavailable: ${err?.message || "unknown error"}`, "error");
    }
  } else {
...
function switchPrimaryTab(tab){
  ...
  if (tab === "scenario"){
    state.primaryTab = "scenario";
  } else if (tab === "stations"){
    state.primaryTab = "stations";
    state.activeTab = "network";
    if (!state.primaryMode) state.primaryMode = "population";
  } else {
    const normalized = (tab === "production") ? "production" : "population";
    state.primaryTab = normalized;
    state.primaryMode = normalized;
    state.lineEditorMode = ui_defaultLineMode();
    if (!state.dynamics) state.dynamics = {};
    state.dynamics.mode = normalized === "production" ? "goods" : "passengers";
...
function ui_openNetworkOverlays(){
  state.panelVisible = true;
  switchPrimaryTab("population");
  switchTab("network");
}

window.ui_openNetworkOverlays = ui_openNetworkOverlays;

function ui_getStationCreateFields(){
  const nameEl = document.getElementById("stationCreateName");
  const latEl = document.getElementById("stationCreateLat");
  const lonEl = document.getElementById("stationCreateLon");
  const popEl = document.getElementById("stationCreatePop");
  return {
    name: nameEl?.value?.trim?.() || "",
    lat: Number(latEl?.value),
    lon: Number(lonEl?.value),
    population: Number(popEl?.value)
  };
}

function ui_setStationFilter(value){
  state.stationFilterTerm = String(value || "");
  updateUI();
}

function ui_convertCityToStation(){
  const node = state.selectedNode;
  if (!node) {
    showToast("Select a node to convert", "warning");
    return;
  }
  if (typeof createStationFromCity !== "function") {
    showToast("Station converter unavailable", "warning");
    return;
  }
  const station = createStationFromCity(node.id, { saveAsCustom: true });
  if (!station) {
    showToast("Failed to convert node to station", "warning");
    return;
  }
  ...
  showToast(`Station ${station.name} ready`, "success");
  updateUI();
}

function ui_toggleStationActive(stationId){
  const targetId = stationId || (state.selectedNode ? nodeToStationId(state.selectedNode.id) : null);
  if (!targetId) {
    showToast("Select a station first", "warning");
    return;
  }
  if (typeof toggleStation !== "function") {
    showToast("Station toggle unavailable", "warning");
    return;
  }
  const isActive = toggleStation(targetId);
  if (typeof markDemandDirty === "function") markDemandDirty();
  if (typeof populateNodesWithStations === "function") populateNodesWithStations();
  showToast(isActive ? "Station reactivated" : "Station disabled", "info");
  updateUI();
}

function ui_createCustomStation(){
  if (typeof registerCustomStation !== "function") {
    showToast("Custom station generator unavailable", "warning");
    return;
  }
  const { name, lat, lon, population } = ui_getStationCreateFields();
  if (!name) {
    showToast("Provide a name for the station", "warning");
    return;
  }
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) {
    showToast("Provide valid coordinates (lat/lon)", "warning");
    return;
  }
  const station = registerCustomStation({ name, lat, lon, population });
  if (!station) {
    showToast("Failed to create custom station", "warning");
    return;
  }
  if (typeof populateNodesWithStations === "function") populateNodesWithStations();
  if (typeof markDemandDirty === "function") markDemandDirty();
  showToast(`Custom station ${station.name} added`, "success");
  updateUI();
}

function ui_startStationPlacement(){
  const { name, population } = ui_getStationCreateFields();
  state.stationPlacementMode = true;
  state.stationPlacementDraft = { name, population };
  showToast("Click the map to place the new station", "info");
  updateUI();
}

function ui_cancelStationPlacement(){
  state.stationPlacementMode = false;
  state.stationPlacementDraft = null;
  updateUI();
}
window.ui_setStationFilter = ui_setStationFilter;
window.ui_convertCityToStation = ui_convertCityToStation;
window.ui_toggleStationActive = ui_toggleStationActive;
window.ui_createCustomStation = ui_createCustomStation;
window.ui_startStationPlacement = ui_startStationPlacement;
window.ui_cancelStationPlacement = ui_cancelStationPlacement;
...
function ui_allowedLineTypes(){
  ...
  return primaryModeIsProduction() ? ["cargo", "mixed"] : ["passenger", "mixed"];
...
function ui_defaultLineMode(){
  ...
  return primaryModeIsProduction() ? "cargo" : "passenger";
...
function opt_buildLinesForScope(nodeIds){
  ...
  const lineType = primaryModeIsProduction() ? "cargo" : "passenger";
  ...
  const buildFallbackLine = (candidateIds, settings, lineIdx) => {
    ...
    const stationStops = (typeof convertNodesToStationIds === "function")
      ? convertNodesToStationIds(routeStops, { createIfMissing: true })
      : routeStops.slice();

    if (stationStops.length < 2) return null;
...
function renderInvestmentPanel(opts = {}){
  ...
  const companyOptions = companies.map(c => `
    ...
    <option value="${c.id}">${escapeHtml(c.name)} â€” ${formatCurrency(Number(c.sharePrice || 0))}/share</option>
  ...
  const companyListHtml = companies.length ? companies.map(c => `
    ...
    <div class="item" style="cursor:default;">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
        ...
        <div style="min-width:0;">
          ...
          <div style="font-size:12px;font-weight:900;color:#64748b;">
            ...
            ${escapeHtml(c.scope || "company")} Â· ${escapeHtml(c.sector || "general")} Â· ${Number(c.marketCap || 0) >= publicMinCap ? "Listed" : "Private"}
      ...
      <div style="margin-top:6px;font-size:12px;font-weight:900;color:#64748b;">
        ...
        Market cap: ${formatCurrency(Number(c.marketCap || 0))} Â· Shares: ${fmtNum(Math.round(Number(c.sharesOutstanding || 0)))}

        ${Number(c.playerShares || 0) > 0 ? ` Â· You: ${fmtNum(Math.round(Number(c.playerShares || 0)))} shares` : ""}
...
const totalPopulation = cells.reduce((sum, cell) => sum + Math.max(0, Number(cell?.pop || 0)), 0);
const totalJobs = cells.reduce((sum, cell) => sum + Math.max(0, Number(cell?.jobs || cell?.production || 0)), 0);
const cellsAllocated = state.catchmentByCell?.size || 0;
const coveragePct = cellCount ? Math.round((cellsAllocated / cellCount) * 100) : 0;

const stationLoads = Array.from(state.stationLoad?.entries() || []);
const totalStationPax = stationLoads.reduce((sum, [, load]) => sum + Math.max(0, Number(load?.pax || 0)), 0);
const totalStationFreight = stationLoads.reduce((sum, [, load]) => sum + Math.max(0, Number(load?.freight || 0)), 0);
const stationCount = stationLoads.length;
const topStationList = stationLoads
  .map(([stationId, load]) => ({
    stationId,
    ratio: Number(load?.loadRatio || 0),
    name: state.stations?.get(String(stationId))?.name || stationId
  }))
  .sort((a, b) => b.ratio - a.ratio)
  .slice(0, 5)
  .map(entry => `${escapeHtml(entry.name)} (${Math.round((entry.ratio || 0) * 100)}% load)`)
  .join(", ") || "n/a";

const coverageSummary = cellCount ? `${cellsAllocated} / ${cellCount} (${coveragePct}%)` : "n/a";

const cellSummaryHtml = `
  <div class="section">
    <div style="font-weight:1000;color:#0f172a;margin-bottom:8px;">Comarca demand</div>
    <div style="line-height:1.5;">
      ${kv("Comarcas tracked", fmtNum(cellCount))}
      <br>${kv("Population", fmtNum(totalPopulation))}
      <br>${kv("Jobs estimate", fmtNum(totalJobs))}
      <br>${kv("Allocated comarcas", coverageSummary)}
    </div>
  </div>
`;

const stationSummaryHtml = `
  <div class="section">
    <div style="font-weight:1000;color:#0f172a;margin-bottom:8px;">Station throughput</div>
    <div style="line-height:1.5;">
      ${kv("Stations reporting", fmtNum(stationCount))}
      <br>${kv("Passengers allocated", fmtNum(totalStationPax))}
      <br>${kv("Freight allocated", fmtNum(totalStationFreight))}
      <br>${kv("Top stations", topStationList)}
    </div>
  </div>
`;
    ...
    ${activeNode ? (() => {
      ...
      return `
        ...
        <div style="font-weight:900;color:#334155;font-size:12px;margin-bottom:6px;">
          ...
          ${escapeHtml(activeNode?.nodeId || activeNodeId)} â€¢ ${escapeHtml(state.nodes?.get(activeNodeId)?.name || "")}
...
const isProd = primaryModeIsProduction();
...
return `
  ...
  <div class="section">
    ...
    <div style="margin-top:10px; line-height:1.7;">
      ...
      ${kv("Delivered goods â‚¬", formatCurrency(s.deliveredGoodsEUR || 0))}

      <br>${kv("Delivered passengers â‚¬", formatCurrency(s.deliveredPassengersEUR || 0))}

      <br>${kv("Lost demand â‚¬", formatCurrency(s.lostDemandEUR || 0))}

      <br>${kv("Congestion penalty â‚¬", formatCurrency(s.congestionPenaltyEUR || 0))}
  ...
  <div class="section">
    ...
    <div style="margin-top:10px; line-height:1.7;">
      ...
      ${kv("Top export", `${s.topExportName || "â€”"} (${formatCurrency(s.topExportValue||0)})`)}

      <br>${kv("Top need", `${s.topNeedName || "â€”"} (${formatCurrency(s.topNeedValue||0)})`)}
    ...
    <div class="hint" style="margin-top:10px;">
      ...
      When we add real â€œgoods typesâ€, this becomes a per-good export/need list.
  ...
  <div class="section">
    ...
    <div style="margin-top:10px; line-height:1.7;">
      ...
      ${kv("Top bottleneck", `${s.topBottleneck || "â€”"}`)}
...
host.innerHTML = `
  ...
  <div style="font-weight:900;color:#334155;font-size:12px;line-height:1.5;">
    ...
    Î² (travel-time sensitivity): <b>${beta}</b><br/>
  ...
  <div style="margin-top:10px;font-size:12px;color:#64748b;font-weight:900;">
    ...
    ${top.map(x => {
      ...
      return `<div>â€¢ ${escapeHtml(name)} â€” <b>${Math.round(x.v)}</b></div>`;
  ...
  <div style="margin-top:10px;font-size:12px;color:#64748b;font-weight:900;">
    ...
    Next: monthly relocation (Î”pop/Î”jobs) will be shown here.
...
function updateUI(){
  ...
  if (primaryModeIsProduction() && state.dynamics.mode !== "goods") state.dynamics.mode = "goods";

  if (!primaryModeIsProduction() && state.dynamics.mode !== "passengers") state.dynamics.mode = "passengers";
  ...
  <div style="font-weight:1000;color:#0f172a">${ui_lineDisplayName(l)}${l.circular ? " âŸ³" : ""}</div>

  <div style="font-size:12px;font-weight:900;color:#64748b"> 

  ${l.stops.length} stops â€¢ ${l.type} 
...
`<option value="" ${!state.activeLine ? "selected" : ""}>(No line selected â€” Inspect)</option>`,

...editorLines.map(l => `<option value="${l.id}" ${l.id===state.activeLine ? "selected" : ""}>${ui_lineDisplayName(l)} â€¢ ${l.type}</option>`)
...
const statusTypes = primaryModeIsProduction() ? ["cargo"] : ["passenger"];
...
const lineEditorTabsHtml = `
  ...
  <div class="tabs" style="margin-bottom:8px;">
    ...
    <div class="tab active" style="cursor:default;">
      ...
      ${primaryModeIsProduction() ? "Cargo lines" : "Passenger lines"}
  ...
  <div class="section">
    ...
    <div style="margin-top:8px;color:#64748b;font-weight:850;font-size:12px;line-height:1.35;">
      ...
      ${activeLineObj ? (() => {
        ...
        return `${capTxt} â€¢ Speed: <b>${Math.round(kmh)}</b> km/h â€¢ Fleet maintenance: <b>${formatCurrency(maint)}</b>/year â€¢ Line Cost (day): <b>${kv("Op cost/day", formatCurrency(lineCostDay))}</b>
    ...
    ${activeLineObj ? (() => {
      ...
      const fmt = (x, suf="") => (x==null || !isFinite(x)) ? "â€”" : `${(x>=100 ? Math.round(x) : x.toFixed(1))}${suf}`;
      ...
      return `
        ...
        <div class="section" style="margin-top:10px;">
          ...
          <div style="line-height:1.7;">
            ...
            <br>${kv("Required trains", m.requiredTrains==null ? "â€”" : fmtNum(m.requiredTrains))}
...
` : "";
  ...
  const isPopulationTab = state.primaryTab === "population";
  const showPrimaryPanel = !isScenarioPrimaryTab();
  ...
  const stationTabHtml = (showPrimaryPanel && state.primaryTab === "stations" && state.activeTab === "network") ? renderStationsPanel() : "";
  const productionTabHtml = (showPrimaryPanel && state.primaryTab === "production" && state.activeTab === "network") ? renderProductionTab() : "";

const dynamicsTabHtml = (showPrimaryPanel && state.activeTab==="dynamics" && state.primaryTab !== "stations")
  ...
  (state.scenarioMeta && state.scenarioMeta.title)
    ...
    ? ` â€¢ <span class="badge">${state.scenarioMeta.title}</span>`

    : "";

  const scenarioTabHtml = state.primaryTab === "scenario" ? ui_scenarioPanelHtml() : "";
    ...
    return `
      ...
      <div class="item" style="cursor:default;">
        ...
        <div style="color:#64748b;font-weight:850;font-size:12px;line-height:1.35;">
          ...
          ${daysLeft}d left â€¢ ${jobProgressPct(j)}% â€¢ ${formatCurrency(jobCostLeft(j))} remaining
  ...
  const exporterName = econ.biggestExporter?.id ? (state.nodes.get(econ.biggestExporter.id)?.name || econ.biggestExporter.id) : "â€”";

  const importerName = econ.biggestImporter?.id ? (state.nodes.get(econ.biggestImporter.id)?.name || econ.biggestImporter.id) : "â€”";
  ...
  const _profitScore = Math.round(50 + 50 * Math.tanh(_profit / 200000000)); // Â±â‚¬200M scale
  ...
  const scorePanelHtml = `
    ...
    <div class="hint" style="margin-top:10px;">
      ...
      <div style="margin-top:8px;color:#64748b;font-weight:850;font-size:12px;">
        ...
        Score = weighted mix of profit, demand met, service capacityÃ—network size, and congestion reliability.
  ...
  const reportRunIdShort = escapeHtml(
    ...
    reportRawRunId.length > 32
      ...
      ? `${reportRawRunId.slice(0, 16)}â€¦${reportRawRunId.slice(-12)}`
  ...
  const reportPanelHtml = `
    ...
    <div class="section">
      ...
      <div class="hint" style="font-size:12px;margin-bottom:8px;">
        ...
        Rows collected: ${fmtNum(reportRowCount)} Â· Run ID: ${reportRunId}

        <br>Versions: dataset ${datasetVersion} Â· model ${modelVersion} Â· schema ${schemaVersion}
  ...
  const lineRankingHtml = topLines.length ? topLines.map(r => `
    ...
    <div class="item" onclick="selectLine('${r.id}')">
      ...
      <div style="margin-top:4px;font-size:12px;font-weight:900;color:#64748b;">
        ...
        ${(Number.isFinite(r.km) ? r.km : 0).toFixed(1)} km â€¢ ${fmtNum(Math.round(r.freq || 0))}/day â€¢ cap ${fmtNum(Math.round(r.vehCap || 0))}
...
const topCellsDetailHtml = underservedEntries.slice(0,3).map(([cellId, value]) => {
  const cell = state.cells?.get(cellId);
  const label = cell?.name || cellId;
  const amount = fmtNum(Math.round(value || 0));
  const safeCellId = escapeHtml(cellId);
  return `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-top:4px;font-size:12px;">
      <span>${escapeHtml(label)} â€” ${amount} underserved</span>
      <button class="btn ghost" style="padding:3px 8px;font-size:11px;" onclick="ui_centerOnCell('${safeCellId}')">Focus</button>
    </div>
  `;
}).join("") || `<div class="hint" style="font-size:12px;">No underserved cells recorded yet.</div>`;
...
const lineCounts = (typeof computeStationLineCounts === "function")
  ? computeStationLineCounts()
  : new Map();
const unusedStations = (Array.from(state.stations?.values() || [])).filter(station => station && station.id && ((lineCounts.get(station.id) || 0) === 0));
const unusedStationsListHtml = unusedStations.slice(0, 20).map(station => `
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
          <div style="flex:1;">
            <div style="font-weight:900;color:#0f172a;">${escapeHtml(station?.name || station.id)}</div>
            <div class="hint" style="font-size:11px;margin-top:2px;">Pop: ${fmtNum(Math.round(station?.coveredPopulation || 0))} people Â· ${fmtNum(lineCounts.get(station.id) || 0)} lines</div>
          </div>
          <button class="btn secondary" style="width:auto;padding:4px 10px;font-size:11px;" onclick="ui_centerOnNodeId('${escapeHtml(station.id)}')">Focus</button>
        </div>
`).join("") || `<div class="hint" style="font-size:12px;">All stations have service.</div>`;
const unusedSummary = `${fmtNum(unusedStations.length)} unused station${unusedStations.length === 1 ? "" : "s"}`;
const infraStatus = (typeof getRealInfraStatus === "function")
  ? getRealInfraStatus()
  : (state.realInfra || {});
...
function renderStationsPanel(){
  ...
  const custom = stations.filter(s => String(s.source || '').toLowerCase().includes('custom')).length;
  const filterTerm = (state.stationFilterTerm || '').trim().toLowerCase();
  const filtered = filterTerm
    ? stations.filter(s => String(s.name || s.id || '').toLowerCase().includes(filterTerm))
    : stations;
  const sorted = filtered.slice().sort((a,b) => {
    const aRatio = Number(state.stationLoad?.get(a.id)?.loadRatio || 0);
    const bRatio = Number(state.stationLoad?.get(b.id)?.loadRatio || 0);
    if (bRatio !== aRatio) return bRatio - aRatio;
    return (Number(b.population || 0) - Number(a.population || 0));
  });
  const linesPerStation = new Map();
  for (const line of state.lines.values()){
    ...
    for (const stop of line.stops){
      ...
      seen.add(stop);
      linesPerStation.set(stop, (linesPerStation.get(stop) || 0) + 1);
    }
  }
  const limited = sorted.slice(0, 60);
  const listHtml = limited.map(station => {
    const load = state.stationLoad?.get(station.id) || {};
    const ratio = Math.round((load.loadRatio || 0) * 100);
    const cityAlloc = Array.from(state.stationCityAllocations?.get(station.id) || []);
    const topCities = cityAlloc
      .slice(0, 3)
      .map(entry => {
        const name = entry.cityName || entry.cityId || "unknown";
        const pop = fmtNum(Math.round(entry.share || 0));
        const focusLink = entry.cityNodeId
          ? `<span style="color:#2563eb;cursor:pointer;font-weight:600;" onclick="ui_centerOnNodeId('${escapeHtml(entry.cityNodeId)}')">[view]</span>`
          : "";
        return `${escapeHtml(name)} ${pop} ${focusLink}`;
      })
      .join(" Â· ");
    const linesCount = linesPerStation.get(station.id) || 0;
    return `
      <div class="item" style="display:flex;flex-direction:column;gap:6px;">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
          <div style="min-width:0;">
            <div style="font-weight:1000;color:#0f172a;">${escapeHtml(station.name || station.id)}</div>
            <div style="font-size:12px;color:#64748b;">
              ${escapeHtml(String(station.source || 'auto').toUpperCase())} Â· ${linesCount} lines Â· ${station.active ? 'Active' : 'Disabled'}
            </div>
          </div>
          <div style="display:flex;gap:6px;">
            <button class="btn secondary" style="width:auto;padding:4px 10px;" onclick="ui_centerOnNodeId('${escapeHtml(station.id)}')">Focus</button>
            <button class="btn ${station.active ? 'warning' : 'success'}" style="width:auto;padding:4px 10px;" onclick="ui_toggleStationActive('${escapeHtml(station.id)}')">
              ${station.active ? 'Disable' : 'Enable'}
            </button>
          </div>
        </div>
        <div style="display:flex;flex-wrap:wrap;gap:10px;font-size:12px;color:#0f172a;">
          <span>${fmtNum(Math.round(load.pax || 0))} pax/day</span>
          <span>${fmtNum(Math.round(load.freight || 0))} freight/day</span>
          <span>Load ${ratio}%</span>
        </div>
        ${topCities ? `<div style="font-size:11px;color:#475569;">Top draws: ${topCities}</div>` : ""}
      </div>
    `;
  }).join('') || `<div class="hint">No matching stations.</div>`;
  const placementActive = !!state.stationPlacementMode;
  const draft = state.stationPlacementDraft || {};
  const filterValue = escapeHtml(state.stationFilterTerm || '');
  return `
    <div class="section">
      <div style="font-weight:1000;color:#0f172a;margin-bottom:8px;">Stations overview</div>
      <div style="line-height:1.6;">
        ${kv('Stations tracked', fmtNum(total))}
        <br>${kv('Active', fmtNum(active))} Â· ${kv('Disabled', fmtNum(disabled))}
        <br>${kv('Custom entries', fmtNum(custom))}
      </div>
    </div>
    <div class="section">
      <div class="row" style="gap:8px;flex-wrap:wrap;">
        <input class="field" type="text" placeholder="Filter stations" value="${filterValue}" oninput="ui_setStationFilter(this.value)">
        <button class="btn secondary" style="width:auto;padding:8px 10px;margin-top:0;" onclick="ui_setStationFilter('')">Clear</button>
      </div>
      <div class="list" style="margin-top:8px;">${listHtml}</div>
      <div class="hint" style="margin-top:6px;font-size:11px;">
        Showing ${limited.length} of ${filtered.length || stations.length} ${filtered.length === stations.length ? 'stations' : 'filtered stations'}.
      </div>
    </div>
    <div class="section">
      <div style="font-weight:1000;color:#0f172a;margin-bottom:6px;">Create or place station</div>
      <input id="stationCreateName" class="field" placeholder="Station name" value="${escapeHtml(draft.name || '')}">
      <div class="row" style="margin-top:8px;gap:8px;">
        <input id="stationCreateLat" class="field" type="number" placeholder="Latitude">
        <input id="stationCreateLon" class="field" type="number" placeholder="Longitude">
      </div>
      <div class="row" style="margin-top:8px;gap:8px;">
        <input id="stationCreatePop" class="field" type="number" placeholder="Population">
        <button class="btn secondary" style="width:auto;padding:8px 12px;margin-top:0;" onclick="ui_createCustomStation()">Create now</button>
      </div>
      <div class="row" style="margin-top:8px;gap:8px;">
        <button class="btn" style="width:auto;padding:8px 12px;margin-top:0;" onclick="ui_startStationPlacement()">Place on map</button>
        ${placementActive ? `<button class="btn danger" style="width:auto;padding:8px 12px;margin-top:0;" onclick="ui_cancelStationPlacement()">Cancel placement</button>` : ''}
      </div>
      ${placementActive ? `<div class="hint" style="margin-top:6px;">Click the map to drop the station ${escapeHtml(draft.name || '(name optional)')}. Only one station can be placed at a time.</div>` : ''}
    </div>
  `;
}
...
const overlaySectionHtml = `
      <div class="section">
        <div style="font-weight:1000;color:#0f172a;margin-bottom:8px;">Demand overlays</div>
        <div class="hint" style="margin-bottom:8px;font-size:11px;">${infraLabel} Â· ${infraHint}</div>
        <div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:6px;">
          ...
          <label style="font-weight:900;color:#334155;display:flex;gap:6px;align-items:center;">
            <input type="checkbox" ${mapLayers.showStations ? "checked" : ""} onchange="setMapLayerOption('showStations', this.checked)">
            Station nodes
          </label>
          <label style="font-weight:900;color:#334155;display:flex;gap:6px;align-items:center;">
            <input type="checkbox" ${mapLayers.showRealInfra ? "checked" : ""} onchange="setMapLayerOption('showRealInfra', this.checked)">
            Real infrastructure overlay
          </label>
          <label style="font-weight:900;color:#334155;display:flex;gap:6px;align-items:center;">
            <input type="checkbox" ${mapLayers.showCountryBorders ? "checked" : ""} onchange="setMapLayerOption('showCountryBorders', this.checked)">
            Country borders
          </label>
          <label style="font-weight:900;color:#334155;display:flex;gap:6px;align-items:center;">
            <input type="checkbox" ${mapLayers.highlightUnusedStations ? "checked" : ""} onchange="setMapLayerOption('highlightUnusedStations', this.checked)">
            Highlight unused stations
          </label>
        </div>
        <div style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin-bottom:10px;">
          <label style="font-weight:900;color:#334155;display:flex;gap:6px;align-items:center;">
            <input type="checkbox" ${mapLayers.showClusters ? "checked" : ""} onchange="setMapLayerOption('showClusters', this.checked)">
            Legacy cluster dots
          </label>
          <label style="font-weight:900;color:#334155;display:flex;gap:6px;align-items:center;">
            <input type="checkbox" ${mapLayers.showCities ? "checked" : ""} onchange="setMapLayerOption('showCities', this.checked)">
            Legacy city dots
          </label>
        </div>
        <div style="display:flex;gap:8px;align-items:center;">
        ...
        <div class="section" style="margin-top:12px;padding-top:12px;border-top:1px solid #e2e8f0;">
          <div style="display:flex;align-items:center;justify-content:space-between;">
            <div style="font-weight:1000;color:#0f172a;">${unusedSummary}</div>
            <span class="hint" style="margin:0;font-size:11px;">Showing ${fmtNum(Math.min(unusedStations.length, 20))} of ${fmtNum(unusedStations.length)}</span>
          </div>
          <div style="max-height:220px;overflow:auto;margin-top:6px;">
            ${unusedStationsListHtml}
          </div>
        </div>
      </div>
`;
...
const selectedCell = state.selectedCellId ? state.cells?.get(state.selectedCellId) : null;
const selectedStationId = selectedNode && typeof nodeToStationId === "function"
  ? nodeToStationId(selectedNode.id)
  : null;

const selectionInspectorHtml = (() => {
  if (selectedCell) {
    const underservedValue = Number(state.underservedByCell?.get(selectedCell.id) || 0);
    const bestStationId = state.catchmentByCell?.get(selectedCell.id);
    const bestStationName = bestStationId ? (state.stations?.get(bestStationId)?.name || bestStationId) : "n/a";
    const allocations = Array.from(state.cellToStationAllocation?.get(selectedCell.id) || []);
    allocations.sort((a,b) => (Number(b?.share || 0)) - (Number(a?.share || 0)));
    const topAllocationsHtml = allocations.length
      ? allocations.slice(0,3).map(entry => {
        const station = state.stations?.get(String(entry?.stationId));
        const label = station?.name || entry?.stationId || "unknown";
        const sharePct = Math.round((entry?.share || 0) * 100);
        return `<div style="font-size:12px;line-height:1.4;">${escapeHtml(label)} Â· ${sharePct}% Â· ${fmtNum(Math.round(entry?.pax || 0))} pax/day</div>`;
      }).join("")
      : `<div class="hint" style="font-size:12px;">No allocations yet.</div>`;
    return `
      <div class="section">
        <div style="font-weight:1000;color:#0f172a;margin-bottom:8px;">Cell setup: ${escapeHtml(selectedCell.name)}</div>
        <div class="hint" style="line-height:1.6;">
          ${kv("Population", fmtNum(selectedCell.pop))}
          <br>${kv("Jobs", fmtNum(selectedCell.jobs || 0))}
          <br>${kv("Underserved", fmtNum(Math.round(underservedValue)))}
          <br>${kv("Best station", escapeHtml(bestStationName))}
        </div>
        <div style="margin-top:8px;">
          <div class="k" style="margin-bottom:4px;">Top allocations</div>
          ${topAllocationsHtml}
        </div>
      </div>
    `;
  ...
  if (!selectedNode) {
    return `
      <div class="section">
        <div class="hint">Select a station node or click a comarca cell to inspect demand/service details.</div>
      </div>
    `;
  }

  if (!selectedStationId) {
    const cityKey = typeof cityKeyFromCity === "function"
      ? cityKeyFromCity(selectedNode)
      : (selectedNode.id || selectedNode.name || "");
    const cityAlloc = cityKey ? (state.cityToStationAllocation?.get(cityKey) || []) : [];
    const cityPop = Math.max(0, Number(selectedNode.population ?? selectedNode.pop ?? selectedNode.pob ?? 0));
    const servedPop = cityAlloc.reduce((sum, entry) => sum + (Number(entry.share) || 0), 0);
    const cityStationsHtml = cityAlloc.length
      ? cityAlloc.slice(0, 3).map(entry => {
        const station = state.stations?.get(String(entry.stationId));
        const name = station?.name || entry.stationId || "unknown";
        const sharePct = Math.round((Number(entry.sharePct) || 0) * 100);
        const stationIdJs = JSON.stringify(entry.stationId || "");
        return `
          <div style="font-size:12px;line-height:1.4;display:flex;align-items:center;gap:6px;">
            <span>${escapeHtml(name)} â€” ${fmtNum(Math.round(entry.share || 0))} ppl (${sharePct}%)</span>
            <button class="btn ghost" style="padding:2px 6px;font-size:11px;" onclick="ui_centerOnNodeId(${stationIdJs})">Focus</button>
          </div>
        `;
      }).join("")
      : `<div class="hint" style="font-size:12px;">No stations serving this city yet.</div>`;
    return `
      <div class="section">
        <div class="hint">
          ${escapeHtml(selectedNode.name)} is not a registered station yet.
        </div>
        <div class="hint" style="margin-top:6px;">
          ${kv("Population", fmtNum(cityPop))}
          <br>${kv("Assigned to stations", fmtNum(Math.round(servedPop)))}
        </div>
        <div style="margin-top:8px;">
          <div class="k" style="margin-bottom:4px;">Serving stations (top 3)</div>
          ${cityStationsHtml}
        </div>
        <div class="row" style="gap:6px;margin-top:8px;">
          <button class="btn secondary" onclick="ui_convertCityToStation()">Convert to station</button>
        </div>
      </div>
    `;
  }

  const station = state.stations?.get(String(selectedStationId));
  const stationStats = station ? (state.stationLoad?.get(selectedStationId) || {}) : {};
  const linesServing = selectedStationId
    ? Array.from(state.lines.values()).filter(l => Array.isArray(l?.stops) && l.stops.includes(selectedStationId))
    : [];
  const lineSummary = linesServing.length
    ? linesServing.slice(0,3).map(l => escapeHtml(l.name || l.id)).join(", ")
    : "No lines yet";
  const shareButton = station
    ? `<button class="btn secondary" onclick="ui_toggleStationActive()">${station.active ? "Remove station" : "Reactivate station"}</button>`
    : "";
  const stationAllocations = Array.from(state.stationCityAllocations?.get(selectedStationId) || []);
  const mainCities = stationAllocations.filter(entry => entry.isMain).slice(0, 3);
  const secondaryCities = stationAllocations.filter(entry => !entry.isMain).slice(0, 3);
  const citySharesHtml = [];
  if (mainCities.length) {
    citySharesHtml.push(`<div style="font-size:12px;margin-top:8px;"><b>Main for:</b> ${mainCities.map(entry => `${escapeHtml(entry.cityName || entry.cityId || "unknown")} (${Math.round((entry.share || 0) * 100)}%)`).join(", ")}</div>`);
  }
  if (secondaryCities.length) {
    citySharesHtml.push(`<div style="font-size:12px;margin-top:4px;"><b>Secondary:</b> ${secondaryCities.map(entry => `${escapeHtml(entry.cityName || entry.cityId || "unknown")} (${Math.round((entry.share || 0) * 100)}%)`).join(", ")}</div>`);
  }
  return `
    <div class="section">
      <div style="font-weight:1000;color:#0f172a;margin-bottom:8px;">Station: ${escapeHtml(station?.name || selectedNode.name)}</div>
      <div class="hint" style="line-height:1.6;">
        ${kv("Passengers assigned/day", fmtNum(Math.round(stationStats.pax || 0)))}
        <br>${kv("Freight assigned/day", fmtNum(Math.round(stationStats.freight || 0)))}
        <br>${kv("Load ratio", `${Math.round((stationStats.loadRatio || 0) * 100)}%`)}
        <br>${kv("Congestion penalty", fmtPercentText(stationStats.congestionPenalty))}
        <br>${kv("Lines serving", `${linesServing.length} (${lineSummary})`)}
      </div>
      <div class="row" style="gap:8px;margin-top:8px;">
        ${shareButton}
        <button class="btn secondary" onclick="ui_convertCityToStation()">Convert city</button>
      </div>
      ${citySharesHtml.join("")}
    </div>
  `;
...
const hasPendingInfraChanges = Boolean(state?.dirty?.demand || state?.dirty?.network);
const pendingInfraStatus = hasPendingInfraChanges
  ? `<span style="color:#b45309;font-weight:900;">Pending updates ready â€“ commit to apply</span>`
  : `<span style="color:#15803d;font-weight:900;">Infrastructure synced</span>`;

const networkTabHtml = (showPrimaryPanel && state.activeTab==="network" && state.primaryTab !== "stations")
  ...
  ? (isPopulationTab ? `
      ...
      <div class="section">
        ...
        <div class="section">
          ...
          <div class="row" style="gap:8px;align-items:center;">
            ...
            <button class="btn secondary" style="width:auto;padding:8px 10px;margin-top:0;" onclick="ui_commitPendingChanges()">Commit pending changes</button>

            <div class="hint" style="flex:1;margin:0;font-size:11px;">
      ...
      <div class="section">
        ...
        <div class="hint" style="margin-top:6px;font-size:11px;">
          ...
          Versions: dataset ${datasetVersion} Â· model ${modelVersion} Â· schema ${schemaVersion}
...
` : (overlaySectionHtml + productionTabHtml + selectionInspectorHtml + lineStatusBlockHtml))

  : "";

const builderStatusHtml = (state.debug?.perf && state.trackBuildMode) ? (() => {
  const activeLine = state.activeLine ? state.lines.get(state.activeLine) : null;
  const lineStops = Array.isArray(activeLine?.stops) ? activeLine.stops.length : 0;
  const startName = state.pendingTrackNode ? state.pendingTrackNode.name : "(none)";
  const selectedName = state.selectedNode ? state.selectedNode.name : "(none)";
  const trackModeLabel = state.trackBuildAction === "demolish" ? "Track demolish" : "Track build";
  const lineModeLabel = state.lineBuildMode ? "Line editor" : "Line view";
  return `
        <div class="section" style="font-size:12px;">
          <div style="font-weight:900;color:#0f172a;margin-bottom:4px;">Builder status</div>
          <div style="margin-bottom:4px;">Tool: ${trackModeLabel} Â· ${lineModeLabel}</div>
          <div style="margin-bottom:4px;">Pending start: ${escapeHtml(startName)}</div>
          <div style="margin-bottom:4px;">Selected node: ${escapeHtml(selectedName)}</div>
          <div>Active line stops: ${lineStops}</div>
        </div>`;
})() : "";

const transportTabHtml = (showPrimaryPanel && state.activeTab==="tracks" && state.primaryTab !== "stations") ? `
...
<br>${kv("Supply / day", fmtNum(Math.round(p.supply || 0)))}
    ...
    <div class="hint" style="margin-top:10px;">
      ...
      Platforms will later increase how many trains can be handled without â€œdelayâ€.
...
const economyTabHtml = (showPrimaryPanel && state.activeTab==="economy" && state.primaryTab !== "stations")

  ? (overlayQuickControlsHtml() + (isPopulationTab ? `
      ...
      <div class="section">
        ...
        <div class="hint">
          ...
          Economy now uses <b>flows</b> (passenger-km & ton-km) limited by <b>service capacity</b> (freq Ã— vehicle capacity) and <b>track capacity</b> (lanes).
...
<div>${kv("Busiest station", busiestStationName==="â€”" ? "â€”" : (busiestStationName + " (" + fmtNum(Math.round(busiestMoves)) + "/day)"))}</div>
        ...
        <div style="margin-top:12px;">
          ...
          <div style="margin-top:8px;color:#64748b;font-weight:850;font-size:12px;">
            ...
            Effectiveness = route km Ã— trains/day Ã— vehicle capacity (capacity-km/day). Click a line to select it.
...
` : `
  ...
  const activePrimaryLabel = state.primaryTab === "production" ? "Industry" : (state.primaryTab === "population" ? "Population" : "Scenario");
  ...
  const windowControlsHtml = `
    ...
    <div class="window-controls">
      ...
      <button class="window-control" title="${windowControlLabel}" onclick="${windowControlAction}">â–</button>
  ...
  const panelMinimizedContent = `
    ...
    <div class="panel-minimized">
      ...
      <div>
        ...
        <div style="font-weight:1000;color:#0f172a;">${activePrimaryLabel} â€¢ ${activeSubLabel} minimized</div>
    ...
    <div class="panel-minimized-row" style="font-size:11px;">
      ...
      <span>Versions: dataset ${datasetVersion} Â· model ${modelVersion} Â· schema ${schemaVersion}</span>
...
function bottomTabBarContent(){
  ...
  const primaryRow = primaryTabs.map(tab => {
    ...
    return `
      ...
      <button type="button" class="bottom-primary-tab" onclick="ui_selectPrimaryNav('${tab.id}')" style="display:flex;align-items:center;gap:6px;padding:8px 12px;border-radius:999px;border:1px solid ${border};background:${bg};color:${color};font-weight:900;font-size:12px;box-shadow:0 8px 20px rgba(15,23,42,0.25);">
  ...
  const subRow = subTabOrder.map(name => {
    ...
    const icon = subTabIcons[name] || "â€¢";

    return `

      <button type="button" class="bottom-sub-tab" onclick="ui_selectSubNav('${name}')" style="display:flex;align-items:center;gap:6px;padding:6px 10px;border-radius:14px;border:1px solid ${border};background:${bg};color:${color};font-size:11px;font-weight:700;">
  ...
  const showSubNav = !!(state.panelVisible && !isScenarioPrimaryTab());

  const subRowWrapper = showSubNav ? `

    <div class="bottom-sub-row" style="display:flex;gap:6px;justify-content:center;flex-wrap:wrap;background:rgba(15,23,42,0.95);border-radius:18px;border:1px solid rgba(255,255,255,0.3);padding:8px 12px;box-shadow:0 -12px 24px rgba(15,23,42,0.45);">
      ${subRow}
...
` : "";
  ...
  return `
    ...
    <div class="bottom-primary-row" style="display:flex;gap:6px;justify-content:center;flex-wrap:wrap;">
      ${primaryRow}
...
// Use clusters (works zoomed out) â€“ pick a few big ones
...
if (!n || !map) return;
  ...
  const isProductionTab = primaryModeIsProduction();

  const lc = isProductionTab ? 0 : getNodeLineCount(nodeId);

  const ts = getNodeTrackStats(nodeId);
  const stationInfo = state.stations?.get(String(nodeId));
  const servedPop = Math.max(0, Number(Math.round(stationInfo?.coveredPopulation ?? stationInfo?.population ?? 0)));
  const coveredCities = Math.max(0, Number(stationInfo?.coveredCities ?? 0));
  const coverageKm = Math.max(0, Number(stationInfo?.coverageKm ?? (window.STATION_COVERAGE_KM ?? 10)));
  ...
  if (isProductionTab) {
    ...
    const popLine = servedPop > 0
      ? `Pop served: ${fmtNum(servedPop)} people${coveredCities ? ` across ${fmtNum(coveredCities)} cities` : ""}`
      : `City population: ${fmtNum(pop)} people`;
    const coverageLine = coverageKm > 0
      ? `${coverageKm.toFixed(1)} km radius`
      : "Coverage unknown";
    const connectionLine = `${fmtNum(ts.edges)} edge${ts.edges === 1 ? "" : "s"} Â· ${fmtNum(ts.lanes)} lane${ts.lanes === 1 ? "" : "s"}`;
    ...
    html += `
      ...
      <div style="font-weight:900;color:#334155;font-size:12px;line-height:1.4;">
        ...
        Coverage: ${coverageLine}<br/>

        Lines: ${fmtNum(lc)} line${lc === 1 ? "" : "s"}<br/>
...
<div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(15,23,42,0.08);">
  ...
  <div style="font-weight:900;color:#334155;font-size:12px;line-height:1.4;">
    ...
    Î²: ${(state.luti && state.luti.beta) || 0.045}
...
`;
  ...
  const stationId = stationInfo?.id || nodeId;
  const stationCityAlloc = stationId
    ? Array.from(state.stationCityAllocations?.get(stationId) || [])
    : [];
  if (stationCityAlloc.length) {
    const citiesList = stationCityAlloc
      .slice()
      .sort((a,b) => (Number(b.share || 0) - Number(a.share || 0)))
      .slice(0, 3)
      .map(entry => {
        const name = entry.cityName || entry.cityId || "unknown";
        return `${escapeHtml(name)} ${fmtNum(Math.round(entry.share || 0))} ppl`;
      })
      .join(" Â· ");
    if (citiesList) {
      html += `
        <div style="font-size:11px;color:#475569;margin-top:6px;">
          Main cities served: ${citiesList}
        </div>
      `;
    }
  }
