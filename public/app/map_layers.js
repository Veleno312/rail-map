...
const layers = {
  ...
  countryBorders: L.layerGroup(),

};



let lastStationMarkersKey = "";

let lastTrackOverlayKey = "";

let worldCountryBorders = null;
let worldCountryBordersPromise = null;
...
function initMap(){
  ...
  map.getPane("hoverPane").style.zIndex = 2000; // keep tooltips above everything
...
layers.flowOverlay.addTo(map);
  ...
  layers.countryBorders.addTo(map);
  ...
  map_applyTheme(state.mapTheme || "metro");

  renderCountryBorder();
  renderWorldCountryBorders();
}
...
function loadWorldCountryBordersData(){
  if (worldCountryBorders) return Promise.resolve(worldCountryBorders);
  if (worldCountryBordersPromise) return worldCountryBordersPromise;
  const url = "/data/world/countries.geojson";
  worldCountryBordersPromise = fetch(url, { cache: "no-store" })
    .then(res => {
      if (!res.ok) throw new Error(`Failed to load ${url} (${res.status})`);
      return res.json();
    })
    .then(data => {
      worldCountryBorders = data;
      worldCountryBordersPromise = null;
      return data;
    })
    .catch((err) => {
  ...
  return worldCountryBordersPromise;
}

function renderWorldCountryBorders(){
  const shouldShow = state?.mapTheme === "metro" || !!state?.mapLayers?.showCountryBorders;
  if (!map || !shouldShow) {
    layers.countryBorders.clearLayers();
    return;
  }
  if (!worldCountryBorders) {
    loadWorldCountryBordersData().then((data) => {
      if (data) renderWorldCountryBorders();
    }).catch(() => {});
    return;
  }
  layers.countryBorders.clearLayers();
  L.geoJSON(worldCountryBorders, {
    pane: "borderPane",
    interactive: false,
    style: {
      color: "#f8fafc",
...
function renderRealInfrastructureOverlay(){
  ...
  const showTracks = state.mapLayers?.showTracks !== false;
  ...
  const style = {
    ...
    color: state.mapTheme === "metro" ? "#ffffff" : "#111111",

    weight: zoom >= 9 ? 3.0 : 2.25,
...
function renderStationMarkers(){
  ...
  const showStations = !!state.mapLayers?.showStations;

  const stamp = `${state.stationStamp || 0}-${showStations ? 1 : 0}-${state.mapLayers?.highlightUnusedStations ? 1 : 0}`;

  if (!showStations) {

    layers.stationMarkers.clearLayers();

    lastStationMarkersKey = stamp;

    return;

  }

  if (!state.stations || !state.stations.size) {
  ...
  if (stamp === lastStationMarkersKey) return;
