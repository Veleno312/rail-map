...
export function computeFlows(state /*, map */) {
  ...
  if (!state.dynamics) state.dynamics = dyn;
  ...
  const PAX_TRIPS_PER_LANE_PER_YEAR  = 25000;  // passenger trips / lane / year

  // Demand value constants (placeholders)
  const GOODS_EUR_PER_UNIT = 1; // EUR per goods unit
  const PAX_EUR_PER_TRIP = 2;   // EUR per passenger trip

  const DAYS_PER_YEAR = 365;
  const DEMAND_DECAY_PER_KM = 0.03;
  const PAX_DECAY_PER_KM = 0.02;
  const GOODS_BASE_COEFF = 0.0008;
  const PAX_BASE_COEFF = 0.0012;
...
const top = pool
  ...
  if (!graph || graph.size === 0) {
    ...
    let paxDemand = 0;
    for (const n of top) {
      const prod = Math.max(0, Number(n.production || 0));
      const need = Math.max(0, Number(n.needs || 0));
      goodsDemand += (prod + need) * GOODS_BASE_COEFF;
      paxDemand += Math.max(0, Number(n.population || 0)) * 0.01;
    }
    const goodsInt = Math.max(0, Math.round(goodsDemand));
    const paxInt = Math.max(0, Math.round(paxDemand));
    result.goodsDemand = goodsInt;
    result.goodsByRail = 0;
    result.goodsByOther = goodsInt;
    result.goodsUnmet = goodsInt;
    result.passengerUnmet = paxInt;
    result.lostDemandEUR = goodsInt * GOODS_EUR_PER_UNIT + paxInt * PAX_EUR_PER_TRIP;
  ...
  let goodsDemandNoPath = 0;
  let paxDemandNoPath = 0;
...
const popB = Math.max(0, Number(B.population ?? B.sumPop ?? 0));
    ...
    const distKm = Number.isFinite(pathRes?.distanceKm)
      ? Number(pathRes.distanceKm)
      : haversineKm(A, B);

    // Demand uses a gravity-style decay with distance
    const goodsCore = (prodA + needsB) * GOODS_BASE_COEFF;
    const goodsProxy = (goodsCore > 0) ? 0 : ((popA + popB) * 0.00001);
    const goodsRaw = (goodsCore + goodsProxy) * Math.exp(-DEMAND_DECAY_PER_KM * distKm);

    const paxCore = Math.sqrt(popA * popB) * PAX_BASE_COEFF;
    const paxRaw = paxCore * Math.exp(-PAX_DECAY_PER_KM * distKm);

    const goodsUnits = Math.max(0, goodsRaw);
    const paxTrips = Math.max(0, paxRaw);
    ...
    if (!hasPath) {
      goodsDemandNoPath += goodsUnits;
      paxDemandNoPath += paxTrips;
  ...
  if (maxOverload > 0) {
    ...
    // penalty EUR (separate from "lost demand")
    result.congestionPenaltyEUR += 0.05 * maxOverload;
  }

  // -------------------------
  // 5) Demand met %
  // -------------------------
  // Smooth totals (simple first-order adjustment), then round to integer units
  const smooth = state.dynamics._flowSmooth || {};
  const targetGoodsDemand = goodsDemandWithPath + goodsDemandNoPath;
  const targetPaxDemand = paxDemandWithPath + paxDemandNoPath;

  smooth.goodsDemand = smooth.goodsDemand == null ? targetGoodsDemand : (smooth.goodsDemand + (targetGoodsDemand - smooth.goodsDemand) * SMOOTH_ALPHA);
  smooth.paxDemand = smooth.paxDemand == null ? targetPaxDemand : (smooth.paxDemand + (targetPaxDemand - smooth.paxDemand) * SMOOTH_ALPHA);

  // Recompute delivered totals from flows after congestion scaling
  let goodsDeliveredSum = 0;
  let paxDeliveredSum = 0;
  for (const f of result.flows) {
    const g = Math.max(0, Math.round(Number(f.goodsUnitsDelivered || 0)));
    const p = Math.max(0, Math.round(Number(f.paxTripsDelivered || 0)));
    f.goodsUnitsDelivered = g;
    f.paxTripsDelivered = p;
    goodsDeliveredSum += g;
    paxDeliveredSum += p;
  }

  const goodsDemandInt = Math.max(0, Math.round(smooth.goodsDemand));
  const paxDemandInt = Math.max(0, Math.round(smooth.paxDemand));

  result.goodsDelivered = goodsDeliveredSum;
  result.passengerTrips = paxDeliveredSum;
  result.goodsUnmet = Math.max(0, goodsDemandInt - goodsDeliveredSum);
  result.passengerUnmet = Math.max(0, paxDemandInt - paxDeliveredSum);
  result.goodsDemand = goodsDemandInt;
  result.goodsByRail = goodsDeliveredSum;
  result.goodsByOther = Math.max(0, goodsDemandInt - goodsDeliveredSum);

  result.deliveredGoodsEUR = result.goodsDelivered * GOODS_EUR_PER_UNIT;
  result.deliveredPassengersEUR = result.passengerTrips * PAX_EUR_PER_TRIP;
  totalDemandEUR = goodsDemandInt * GOODS_EUR_PER_UNIT + paxDemandInt * PAX_EUR_PER_TRIP;
  result.lostDemandEUR = Math.max(0, totalDemandEUR - (result.deliveredGoodsEUR + result.deliveredPassengersEUR));
  ...
  state.dynamics._flowSmooth = smooth;
